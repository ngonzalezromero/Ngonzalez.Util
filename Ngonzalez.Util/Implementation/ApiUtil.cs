...
namespace Ngonzalez.Util
{
    public sealed class ApiUtil : IApiUtil
    {
        ...
        private static readonly char[] validUrlCharacter = new char[] { '!', ':', '@', '$' };
        ...
        public string EncriptUrl(string plainText, string seed, string iv)
        {
            var urlTemp = new Aes(seed, iv).Encrypt(plainText);
            var urlClean = urlTemp.Replace('/', validUrlCharacter[0]).Replace('+', validUrlCharacter[1]).Replace('=', validUrlCharacter[2]);
            return urlClean;
        }

        public string DecryptUrl(string cipherText, string seed, string iv)
        {
            try
            {
                var urlTemp = cipherText.Replace(validUrlCharacter[0], '/').Replace(validUrlCharacter[1], '+').Replace(validUrlCharacter[2], '=');
                return new Aes(seed, iv).Decrypt(urlTemp);
            }
            catch (Exception)
            {
                return null;
        ...
        {
            var temp = new Aes().Encrypt(DateTime.Now.Ticks.ToString()).Replace("+", random.Next(1, 9).ToString()).Replace("/", random.Next(1, 9).ToString());
        ...
        public string DateTimeToText(DateTime date)
        {
            ...
            if (diff.Days >= 7)
            {
                return $"{date.ToString("d")} at {hour}";
            }
            else if (diff.Days >= 2)
            {
                return $"{diff.Days} Days ago at {hour}";
            }
            else if (diff.Days == 1)
            {
